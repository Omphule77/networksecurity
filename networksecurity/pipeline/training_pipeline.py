...
from networksecurity.constant.training_pipeline import TRAIN_BUCKET_NAME
...
class TrainingPipeline:
    ...
    ##local articat going to s3 bucket
    def sync_artifact_dir_to_s3(self):
        try:
            aws_bucket_url=f"s3://{TRAIN_BUCKET_NAME}/artifact/{self.training_pipeline_config.timestamp}"
            self.s3_sync.sync_folder_to_s3(folder=self.training_pipeline_config.artifact_dir,aws_bucket_url=aws_bucket_url)
        except Exception as e:
            raise NetworkSecurityException(e,sys) from e

    ##local model going to s3 bucket
    def sync_saved_model_dir_to_s3(self):
        try:
            aws_bucket_url=f"s3://{TRAIN_BUCKET_NAME}/final_model/{self.training_pipeline_config.timestamp}"
